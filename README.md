...
- **Collections:** any folder under `content/` is a collection. `content/guides/<slug>.md` with front matter (`title`, `description`, `date`, `collection`, `cluster`, `source`) becomes `/guides/<slug>/`, gets listed at `/guides/`, in the sitemap, and the newest four appear in the Guides row on the home page. A blog is the same thing: `content/blog/<slug>.md` -> `/blog/`. A page whose front matter has `merged_into: <slug>` (an old guide folded into a newer one) still builds at its URL as a one-line pointer, with a canonical tag and noindex pointing at the merged guide, and is left out of the index, the home row and the sitemap; nothing redirects.
- **Tools:** a markdown page drops in an interactive tool by putting its token on a line of its own; today `{{PORTAFILTER_TOOL}}` (templates/partials/portafilter-tool.html, styles `.pf-*` in storefront.css). The finder also has its own page, `content/portafilter-size-finder.md` -> `/portafilter-size-finder/`, listed in `TEXT_PAGES` in build.mjs.
